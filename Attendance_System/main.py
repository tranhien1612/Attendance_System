...
def contact(): # thông báo
    mess._show(title='Contact us', message="Please contact us on : 'nguyenthucanh@gmail.com' ")

###################################################################################

def check_haarcascadefile(): # Kiểm tra file haarcascade_frontalface_default.xml có trong thư mục làm việc k
...
def save_pass(): # Lưu mật khẩu mới
...
def change_pass(): # Thay đổi mật khẩu
    global master
    master = tk.Tk()
...
def psw(): # Kiểm tra mật khẩu trước khi muốn Save Image
    assure_path_exists("TrainingImageLabel/")
    exists1 = os.path.isfile("TrainingImageLabel\psd.txt")
    if exists1:
        tf = open("TrainingImageLabel\psd.txt", "r")
    ...
    if (password == key):
        mess._show(title='Password', message='Password OK')
        TrainImages()
    elif (password == None):
        pass
    else:
...
def TakeImages():
    ...
    if ((name.isalpha()) or (' ' in name)):
        ...
        detector = cv2.CascadeClassifier("haarcascade_frontalface_default.xml") # Đặc trưng khuôn mặt được lưu vào biến detector
        sampleNum = 0
        while (True): # đúng khi camera đã bật
            ret, img = cam.read() # Lưu ảnh từ camera vào biến img
            gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY) # Chuyển đổi từ ảnh màu sang ảnh xám
            faces = detector.detectMultiScale(gray, 1.3, 5) # Khuôn mặt được phát hiện trên bức ảnh lưu vào biến faces
            for (x, y, w, h) in faces: # Vẽ hình chữ nhật bao quanh khuôn mặt
...
def TrainImages(): # Tranning dữ liệu
    check_haarcascadefile()
    assure_path_exists("TrainingImageLabel/")
    recognizer = cv2.face_LBPHFaceRecognizer.create()
    harcascadePath = "haarcascade_frontalface_default.xml"
    detector = cv2.CascadeClassifier(harcascadePath)
    faces, ID = getImagesAndLabels("TrainingImage") # lấy dữ liệu khuôn mặt và ID từ ảnh tranning từ thư mục TrainingImage
    try:
        recognizer.train(faces, np.array(ID))
    except:
        mess._show(title='No Registrations', message='Please Register someone first!!!')
        return
    recognizer.save("TrainingImageLabel\Trainner.yml") # Dữ liệu sau khi Tranning được lưu vào file Trainner.yml
...
def TrackImages(): # Phát hiện khuôn mặt trong bức ảnh
    check_haarcascadefile() # Kiểm tra có file đặc trưng của khuôn mặt (haarcascade_frontalface_default.xml) hay không
    ...
    if exists1:
        ...
        window.destroy()
    while True:
        ret, im = cam.read() # Đọc ảnh từ camera lưu vào biến im
        gray = cv2.cvtColor(im, cv2.COLOR_BGR2GRAY) # chuyển đổi từ ảnh màu sang ảnh xám
        faces = faceCascade.detectMultiScale(gray, 1.2, 5) # Đặc trưng của khuôn mặt được lưu vào biến faces
        for (x, y, w, h) in faces: # Vẽ hình chữ nhật bao quanh khuôn mặt
        ...
        if (cv2.waitKey(1) == ord('q')): # Nhấn q để tắt camera
            break
    ts = time.time()
    date = datetime.datetime.fromtimestamp(ts).strftime('%d-%m-%Y')
